const products = [
  ...
  const productsTypeChip = products.filter((product) => {
    const isChip = product.type === `chip`
    return isChip
  })
  console.log(productsTypeChip)

  const productsTypeBeer = products.filter((product) => {
    const isbeer = product.type === `beer`
    return isbeer


   })
   console.log(productsTypeBeer);

   const productsPrecioMayor = products.filter((product) =>{
       const isMayor = product.price >= 50
       return isMayor

   })
   console.log(productsPrecioMayor)
   
   const productsPrecioMenor = products.filter((product) =>{
       const isMenor = product.price <= 50
       return isMenor
   })
   console.log(productsPrecioMenor)
...
const Menor20 = products.map((product) =>{
    if(product.price<20){
        product.ican = true
